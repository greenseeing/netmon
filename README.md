...
One colour-coded feed shows every DNS / SNI / HTTP / flow event as it happens (kind, direction, host, detail) with the newest at the top, alongside panels for top hosts, per-kind counts, an events/sec sparkline, and capture health (queue depth, drops). The columns resize to fit any terminal width. Keys: `q` quit, `space` pause, `f` open the filter bar, ↑/↓ to inspect a row's full record, `g` to follow the newest again. The filter bar has a checkbox per **kind**, **direction** and **scope** — tick several within a group (DNS *and* TLS), and the groups combine (only internet-bound DNS). `a`/`n` tick or clear a whole group, `space` toggles one, `f` or `esc` closes the bar. A constrained filter is named in the feed's border, so a filtered feed can never be mistaken for a quiet network. Scrolling down or selecting a row freezes the feed so you can read history without it snapping back; `g` resumes the live tail. With `--log`, the JSONL record plus a `netmon.log` diagnostic are written to `logs/run-*/` underneath while the dashboard owns the screen; without it the view is purely ephemeral.
